...
const INJECTED_STYLES = `
  ...
  /* INSIDE THE CARD: Dark text for the light background */
  .text-card-silver-matte {
      background: linear-gradient(180deg, #0065F4 0%, #003D99 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
      transform: translateZ(0);
      filter: 
          drop-shadow(0px 4px 12px rgba(0,101,244,0.3)) 
          drop-shadow(0px 2px 4px rgba(0,101,244,0.2));
  }

  /* Deep Physical Card with Dynamic Mouse Lighting */
  .premium-depth-card {
      background: linear-gradient(145deg, #D8EAFF 0%, #C5DCFF 100%);
      box-shadow: 
  ...
  return (
    ...
    >
      ...
      <div className="hero-text-wrapper absolute z-10 flex flex-col items-center justify-center text-center w-screen px-4 will-change-transform transform-style-3d">
        {logoSrc && (
          <div className="hero-logo gsap-reveal mb-8">
            <Image 
              src={logoSrc}
              alt="Logo"
              width={200}
        ...
        {heroDescription && (
          <p className="hero-description gsap-reveal mt-8 text-sm md:text-base lg:text-lg text-muted-foreground max-w-4xl leading-relaxed px-4 font-light">
            {heroDescription}
          </p>
        )}
      </div>
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto px-4 lg:px-12 flex flex-col justify-evenly lg:grid lg:grid-cols-3 items-center lg:gap-8 z-10 py-6 lg:py-0">
            ...
            <div className="mockup-scroll-wrapper order-2 lg:order-2 relative w-full h-[420px] lg:h-[680px] flex items-center justify-center z-10" style={{ perspective: "1000px" }}>
              ...
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                ...
                {deviceType === 'laptop' ? (
                  /* Laptop Mockup with Outer Timeline */
                  <div
                    ref={mockupRef}
                    className="relative will-change-transform transform-style-3d"
                  >
                    {/* Outer Circular Timeline - Around the Laptop */}
                    {timelineSteps && timelineSteps.length > 0 && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ width: '800px', height: '800px', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
                        {/* Background Circle - connects all dots */}
                        <svg className="absolute w-full h-full" viewBox="0 0 800 800" aria-hidden="true">
                          <circle cx="400" cy="400" r="380" fill="none" stroke="rgba(59,130,246,0.2)" strokeWidth="2" />
                          <circle className="timeline-connector" cx="400" cy="400" r="380" fill="none" stroke="#3B82F6" strokeWidth="2" strokeDasharray="2388" strokeDashoffset="2388" style={{ transform: 'rotate(30deg)', transformOrigin: 'center' }} />
                        </svg>
                        
                        {/* Timeline Nodes */}
                        {timelineSteps.map((step, index) => {
                          // Start at 30 degrees, distribute evenly
                          const startAngle = (30 * Math.PI) / 180; // 30 degrees in radians
                          const angle = startAngle + (index / timelineSteps.length) * 2 * Math.PI;
                          const radius = 380; // circle radius
                          ...
                          return (
                            <div
                              key={index}
                              className="timeline-node absolute pointer-events-auto"
                              style={{
                                left: `${x}px`,
                                top: `${y}px`,
                            ...
                            >
                              <div className="relative group">
                                {/* Node Circle */}
                                <div className="w-4 h-4 rounded-full bg-blue-400 shadow-[0_0_20px_rgba(59,130,246,0.8)] hover:scale-150 transition-transform cursor-pointer" />
                                
                                {/* Tooltip on Hover */}
                                <div className="absolute z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap"
                                  style={{
                                    left: isRight ? '80px' : 'auto',
                                    right: isRight ? 'auto' : '80px',
                                    top: '50%',
                                    transform: 'translateY(-50%)'
                                  }}
                                >
                                  <div className="bg-black/80 backdrop-blur-xl border border-blue-400/30 rounded-xl px-4 py-3 shadow-2xl">
                    ...
                    {/* Laptop Screen */}
                    <div className="relative w-[500px] h-[320px] bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] rounded-t-2xl border-[8px] border-[#1a1a1a] shadow-2xl">
                      {/* Screen Content */}
                      <div className="absolute inset-0 bg-[#050914] rounded-lg overflow-hidden shadow-[inset_0_0_30px_rgba(0,0,0,0.9)]">
                        <div className="absolute inset-0 screen-glare pointer-events-none" aria-hidden="true" />
                        ...
                        <div className="relative w-full h-full p-6 flex flex-col">
                          ...
                          <div className="phone-widget flex justify-between items-center mb-4">
                            <div className="flex flex-col">
                              ...
                              <span className="text-2xl font-bold tracking-tight text-white drop-shadow-md">Process Flow</span>
                            </div>
                            <div className="flex gap-2">
                              <div className="w-10 h-10 rounded-xl bg-white/5 text-neutral-200 flex items-center justify-center font-bold text-sm border border-white/10 shadow-lg">AI</div>
                            </div>
                          </div>

                          {/* Main Content - Simple Dashboard */}
                          <div className="flex-1 flex items-center justify-center">
                            <div className="phone-widget relative w-48 h-48 flex items-center justify-center drop-shadow-[0_15px_35px_rgba(0,0,0,0.9)]">
                              <svg className="absolute inset-0 w-full h-full" aria-hidden="true">
                                <circle cx="96" cy="96" r="70" fill="none" stroke="rgba(255,255,255,0.03)" strokeWidth="14" />
                                <circle className="progress-ring" cx="96" cy="96" r="70" fill="none" stroke="#3B82F6" strokeWidth="14" />
                              </svg>
                              <div className="text-center z-10 flex flex-col items-center">
                                <span className="counter-val text-5xl font-extrabold tracking-tighter text-white">0</span>
                                <span className="text-[10px] text-blue-200/50 uppercase tracking-[0.12em] font-bold mt-1">{metricLabel}</span>
                              </div>
                            </div>
                          </div>

                          {/* Bottom Info */}
                          <div className="phone-widget widget-depth rounded-xl p-3 mt-4">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500/20 to-blue-600/5 flex items-center justify-center border border-blue-400/20 shadow-inner">
                                  <svg className="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                                  </svg>
                                </div>
                                <div className="flex-1">
                                  <div className="h-2.5 w-24 bg-neutral-300 rounded-full mb-1.5 shadow-inner" />
                                  <div className="h-2 w-16 bg-neutral-600 rounded-full shadow-inner" />
                                </div>
                              </div>
                      ...
                      {/* Camera notch */}
                      <div className="absolute top-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-[#0a0a0a] border border-[#2a2a2a]" />
                    </div>
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-[#0065F4] text-xs lg:text-sm font-bold tracking-tight">1 Year Streak</p>
                    <p className="text-[#0065F4]/50 text-[10px] lg:text-xs font-medium">Milestone unlocked</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-[#0065F4] text-xs lg:text-sm font-bold tracking-tight">Sponsor Update</p>
                    <p className="text-[#0065F4]/50 text-[10px] lg:text-xs font-medium">Shared successfully</p>
                  </div>
                </div>
